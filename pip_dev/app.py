...
class VersionCheck:
    ...
    def print(self, fmt: str, **kwargs) -> str:
        if fmt == "html":
            ...
            return (
                f'<td colspan="{colspan}" class="{is_phase} {is_valid}">'
                f"<span>{self.version.public}</span>"
                f"</td>"
            )
        return (
            f"{Fore.GREEN if self.is_valid else Fore.RED}"
...
def _generate_versions_for_specifier_set(specifier_set: str) -> List[Version]:
    ...
    for specifier in specifier_set.split(","):
        ...
        if match:
            ...
            vers = _generate_versions_for_version(majors, minors, micros, suffix_num)
...
def _check_generated_versions_for_specifier_set(
    ...
    for version in versions:
        ...
        yield VersionCheck(
            version=version, specifier_set=specifier_set, is_valid=is_valid
        )
...
def generate_versions_table(specifier_set: str, fmt: str = "github") -> str:
    ...
    for base_version in base_versions.values():
        for phase, versions in base_version.items():
            base_version[phase] = sorted(versions, key=lambda x: x.version.public)
    ...
    if fmt != "html":
        ...
        return tabulate(entries, headers=("dev", "pre", "final", "post"), tablefmt=fmt)

    # Count max lengths:
    max_lengths = dict(dev=0, pre=0, final=0, post=0)
    for base_version in base_versions.values():
        for phase in max_lengths.keys():
            max_lengths[phase] = max(max_lengths[phase], len(base_version[phase]))
    ...
    for base_version in base_versions.values():
        ...
        for phase in max_lengths.keys():
            ...
            for version_check in base_version[phase]:
                ...
                cells.append(version_check.print(fmt=fmt, colspan=colspan, phase=phase))
